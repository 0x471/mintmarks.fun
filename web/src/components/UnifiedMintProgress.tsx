...
interface UnifiedMintProgressProps {
  ...
  // SELF Protocol props (CRITICAL - for actual QR verification)
  selfApp?: SelfApp | null
  onSelfVerified?: () => void
  onSelfError?: (error: any) => void
}
...
export function UnifiedMintProgress({
  ...
  // onVerifySelfID, // Not used - QR code handles it directly
  onSignMessage,
  onPayFee,
  onChangeWallet,
  walletAddress,
              ...
              <CardContent className="space-y-4 px-4 sm:px-5 pb-5">
                ...
                {currentStepDetails.requiresAction && (
                  <div className="space-y-3 pt-1">
                    ...
                    {currentStepDetails.key === 'self-id-prompt' && (
                      <div className="space-y-3">
                        <div className="flex items-center gap-2 text-xs p-2.5 rounded-lg border" style={{
                          ...
                          color: 'var(--page-text-secondary)'
                           ...
                           <span>Scan QR code with SELF app to verify you're a unique human</span>
                        </div>

                        {/* SELF Protocol QR Code - CRITICAL INTEGRATION */}
                        {selfApp ? (
                          <div className="flex flex-col items-center gap-3 p-4 rounded-lg border" style={{
                            backgroundColor: 'var(--glass-bg-secondary)',
                            borderColor: 'var(--glass-border)',
                          }}>
                            <SelfQRcodeWrapper
                              selfApp={selfApp}
                              onSuccess={() => {
                                console.log('[UnifiedMintProgress] SELF verification successful!');
                                onSelfVerified?.();
                              }}
                              onError={(error: any) => {
                                console.error('[UnifiedMintProgress] SELF verification error:', error);
                                onSelfError?.(error);
                              }}
                            />
                            <p className="text-xs text-center text-muted-foreground max-w-[200px]">
                              Open SELF app on your phone and scan this QR code
                            </p>
                          </div>
                        ) : (
                          <div className="flex flex-col items-center gap-3 p-4 rounded-lg border" style={{
                            ...
                            <p className="text-xs text-center text-muted-foreground">
                              Generating SELF verification request...
                            </p>
                          </div>
                        )}
                      </div>
