...
export async function searchLumaEmails(
  ...
  const filtered = emailDetails.filter(email => {
    ...
    // Debug logging to understand what emails are being filtered
    console.log('[Gmail Filter] Checking email:', {
      subject: email.subject,
      from: email.from,
      snippet: email.snippet.substring(0, 100),
    });

    const hasIncludeKeyword = config.processing.include.some(keyword => {
      const matches = combined.includes(keyword);
      if (matches) {
        console.log(`[Gmail Filter] ✓ Matched include keyword: "${keyword}"`);
      }
      return matches;
    });

    const hasExcludeKeyword = config.processing.exclude.some(keyword => {
      const matches = combined.includes(keyword);
      if (matches) {
        console.log(`[Gmail Filter] ✗ Matched exclude keyword: "${keyword}" - EXCLUDING`);
      }
      return matches;
    });

    const result = hasIncludeKeyword && !hasExcludeKeyword;
    console.log(`[Gmail Filter] Result: ${result ? 'INCLUDED' : 'EXCLUDED'}`);
