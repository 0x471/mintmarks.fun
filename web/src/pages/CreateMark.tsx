...
import { createSelfApp, handleSelfError } from '@/services/self'
import type { SelfApp } from '@selfxyz/qrcode'
import { mintNFT, getProvider, getSigner, checkVerificationStatus } from '@/services/contracts'
...
export default function CreateMark() {
  ...
  // SELF Protocol State
  const [_selfApp, setSelfApp] = useState<SelfApp | null>(null)
  ...
  const fetchEmails = useCallback(async (token: string, reset: boolean = true) => {
    ...
    try {
      const result: EmailSearchResult = await searchLumaEmails(token, reset ? undefined : nextPageToken, 50)
  ...
  // Removed visibleEmailsCount - now showing all fetched emails immediately
  ...
  // Extract email nullifier from proof (CRITICAL for SELF verification)
  useEffect(() => {
    if (proof && proof.metadata && proof.metadata.emailNullifier) {
      const nullifier = proof.metadata.emailNullifier
      console.log('[CreateMark] Email nullifier extracted:', nullifier.substring(0, 20) + '...')
      setEmailNullifier(nullifier)
    }
  }, [proof])

  // Create SELF app when reaching self-id-prompt step (CRITICAL)
  // BUT first check if email nullifier is already verified
  useEffect(() => {
    if (
      unifiedStep === 'self-id-prompt' &&
      emailNullifier &&
      walletAddress &&
      !_selfApp  // Only create if not already created
    ) {
      // Check if email nullifier is already verified before showing SELF QR
      const checkAndCreateSelf = async () => {
        try {
          const provider = getProvider()
          if (!provider) {
            console.log('[CreateMark] No provider - showing SELF prompt')
            createSelfAppNow()
            return
          }

          console.log('[CreateMark] Checking if email nullifier is already verified...')
          const { isEmailVerified } = await checkVerificationStatus(
            provider,
            emailNullifier,
            walletAddress
          )
          ...
          if (isEmailVerified) {
            console.log('[CreateMark] ✅ Email nullifier already verified - skipping SELF step!')
            addLog('✅ Email already verified with SELF Protocol')
            addLog('Skipping SELF verification step...')
            // Skip SELF - move to verified state and wait for proof
            setUnifiedStep('self-id-verified')
            // Check if proof is ready, if so start minting
            if (proof && proofStatus === 'completed') {
              setTimeout(() => {
            ...
            console.log('[CreateMark] Email nullifier not verified - showing SELF QR')
            createSelfAppNow()
          }
        } catch (error) {
          console.error('[CreateMark] Error checking verification:', error)
          // Default to showing SELF if check fails
          createSelfAppNow()
        }
      }

      const createSelfAppNow = () => {
        console.log('[CreateMark] SELF step reached - creating SELF app')
        addLog('Preparing SELF Protocol verification...')

        try {
          const app = createSelfApp({
            scopeSeed: import.meta.env.VITE_SELF_SCOPE_SEED || 'mintmarks.fun',
            proofOfHumanAddress: CONTRACTS.CELO_SEPOLIA.PROOF_OF_HUMAN || '',
            userId: walletAddress,  // Pass original case from wallet (DO NOT lowercase!)
            emailNullifier: emailNullifier,
            endpointType: 'staging_celo',  // staging_celo for Celo Sepolia testnet
          })

          setSelfApp(app)

          console.log('[CreateMark] SELF app created successfully')
          addLog('SELF QR code ready - scan with SELF app to verify')
        } catch (error) {
          console.error('[CreateMark] Failed to create SELF app:', error)
          addLog('Error: Failed to create SELF verification QR code')
          addLog('Please try again or contact support')
        }
      }

      checkAndCreateSelf()
    }
  }, [unifiedStep, emailNullifier, walletAddress, _selfApp])

  // SELF Protocol handlers
  // Handle SELF verification success
  const handleSelfVerified = () => {
    console.log('[CreateMark] SELF verification successful!')
    addLog('Identity verified successfully via SELF Protocol.')
    setUnifiedStep('self-id-verified')

    // Move to mint step after brief delay
    setTimeout(() => {
      setUnifiedStep('mint-start')
      startMinting()
    }, 1000)
  ...
  // Handle SELF verification error
  const handleSelfVerificationError = (error: any) => {
    console.error('[CreateMark] SELF verification error:', error)

    const errorHandling = handleSelfError(error)

    if (errorHandling.autoSkip) {
      // Email already verified - auto-skip to mint
      console.log('[CreateMark] Auto-skipping SELF verification (already verified)')
      addLog(errorHandling.message || 'Already verified, skipping to mint...')
      setUnifiedStep('self-id-verified')
      setTimeout(() => {
        setUnifiedStep('mint-start')
        startMinting()
      ...
      // Show error to user
      addLog(`SELF verification failed: ${errorHandling.message}`)
      addLog('Please try again or contact support')
    }
  }

  // Handle Self ID Verification - NO LONGER USED (QR code handles it directly)
  const handleVerifySelfID = async () => {
    // This function is kept for backward compatibility but QR code
    // handles verification directly via onSuccess/onError callbacks
    console.log('[CreateMark] handleVerifySelfID called (deprecated)')
  }
  ...
  const startUnifiedFlow = async () => {
    ...
    if (walletAddress || (evmAddress && currentUser)) {
       ...
       // Move to SELF ID step (will check if verification needed inside useEffect)
  ...
  // Start minting process (REAL IMPLEMENTATION)
  const startMinting = async () => {
    if (!proof || !selectedEmail || !walletAddress) {
      setError('Missing proof, email, or wallet address')
      return
    }

    try {
      addLog('[Mint] Starting minting process...')

      // Generate artwork (optional - can be done after minting)
      ...
      addLog(`[Mint] Generating artwork for "${eventName}"...`)
      ...
      addLog('[Mint] Artwork generated successfully')

      // Get provider and signer (CRITICAL)
      addLog('[Mint] Getting wallet provider...')
      const provider = getProvider()
      if (!provider) {
      ...
      addLog('[Mint] Wallet signer obtained')

      // Submit transaction to blockchain (REAL MINTING)
      setUnifiedStep('mint-submitting')
      addLog('[Mint] Submitting transaction to Celo Sepolia...')
      addLog('[Mint] Fee: 0.011 CELO (collection creation + mint)')

      const mintResult = await mintNFT(
        signer,
        proof.proof.proof,
        proof.proof.publicInputs,
        '0.011' // Fee (TODO: calculate dynamically)
      )

      addLog(`[Mint] Transaction confirmed: ${mintResult.txHash}`)
      addLog(`[Mint] Token ID: ${mintResult.tokenId}`)
      addLog(`[Mint] Collection ID: ${mintResult.collectionId}`)
      ...
      const metadata: NFTMetadata = {
        name: `Mark: ${eventName}`,
        description: `Proof of attendance at ${eventName} on ${eventDate}. This Mark represents a zero-knowledge proof of your verified attendance.`,
        ...
        attributes: [
          ...
          { trait_type: 'Token ID', value: mintResult.tokenId },
          { trait_type: 'Collection ID', value: mintResult.collectionId },
        ],
      }

      const nftId = `nft-${Date.now()}-${mintResult.tokenId}`
      ...
      addLog('[Mint] ✨ NFT minted successfully!')
      showToast('NFT minted successfully!', 'success')

    } catch (err: unknown) {
      const error = err as Error
      console.error('[Mint] Failed to mint:', error)
      addLog(`[Mint] ERROR: ${error.message}`)
      setError(error.message || 'Failed to mint NFT')
      setUnifiedStep('mint-start') // Allow retry
    }
  }
          ...
          <TabsContent value="gmail" className="space-y-4">
                ...
                {!fetchingEmails && lumaEmails.length > 0 && (
                  <div className="relative">
                    ...
                    >
                        ...
                        {/* Email count badge */}
                        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border backdrop-blur-md backdrop-saturate-150"
                          style={{
                            backgroundColor: 'var(--modal-bg)',
                            borderColor: 'var(--page-border-color)',
                          }}
            ...
            <UnifiedMintProgress
              ...
              error={error}
                                  ...
                                  selfApp={_selfApp}
                                  onSelfVerified={handleSelfVerified}
                                  onSelfError={handleSelfVerificationError}
                                />
                              </div>
                            )}
                          </div>
                    ...
                    {filteredEmails.length === 0 ? (
                      ...
                      <div className="relative flex-1 flex flex-col min-h-0 mt-2">
                        ...
                        {/* Email list container - showing all fetched emails */}
                        <div
                        ...
                        {filteredEmails.length > 0 && (
                          <div className="mt-4 pt-3 border-t text-center flex-shrink-0" style={{ borderColor: 'var(--page-border-color)', opacity: 0.6 }}>
                            <p className="text-xs font-medium" style={{ color: 'var(--page-text-muted)' }}>
                              Showing {filteredEmails.filter(e => selectedEmail?.id !== e.id).length} {emailFilter !== 'all' ? emailFilter : ''} email{filteredEmails.filter(e => selectedEmail?.id !== e.id).length !== 1 ? 's' : ''}
                              {hasMoreEmails && <span className="ml-1">(scroll down for more)</span>}
                            </p>
                          </div>
                        )}
                      </div>
